...
def filter_reviews_by_skin(df, user_skin_type, user_skin_tone):
    ...
    df['matches_user'] = (df['skintype'] == user_skin_type) & (df['skintone'] == user_skin_tone)
    ...
    filtered_df = df[
        ...
        (~df['productid'].map(product_retain_map))  # Keep all reviews if no match exists
    ].drop(columns=['matches_user'])  # Drop the helper column
...
def get_filtered_products(query,category, min_price, max_price, engine,
                         user_skintype, user_skintone, wanted_ingredients=None, unwanted_ingredients=None):
    """
    ...
    if unwanted_ingredients:
        df = filter_out_ingredients(df, unwanted_ingredients, 'ingredientdesc')
